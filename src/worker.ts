...
function getProtocolVersion(): string {
  const v = (globalThis as unknown as { process?: { env?: Record<string, unknown> } })
...
router.post(MCP_ENDPOINT_PATH, async (request: Request) => {
  ...
  const env =
    (globalThis as unknown as { process?: { env?: Record<string, unknown> } })?.process
      ?.env ?? {};
  const authEnabled = String(env.AUTH_ENABLED || 'false').toLowerCase() === 'true';
  const requireRs = String(env.AUTH_REQUIRE_RS || 'false').toLowerCase() === 'true';
...
router.get('/authorize', async (request: Request) => {
  ...
  const sid =
    url.searchParams.get('sid') || request.headers.get('Mcp-Session-Id') || undefined;
