      ...
      switch (parsed.action) {
        case "list_user": {
          ...
          return ok(
            ...
            {
              limit:
                Number(json.limit ?? parsed.limit ?? 0) || normalized.length,
              offset: Number(json.offset ?? parsed.offset ?? 0) || 0,
              total:
                Number(json.total ?? normalized.length) || normalized.length,
              items: normalized,
            },
            msg
          );
        ...
        case "get": {
          ...
          if (parsed.fields) url.searchParams.set("fields", parsed.fields);
          const response = await http(url.toString(), { headers, signal });
        ...
        case "items": {
          ...
          const tracksWithPositions = items
            ...
            .map(({ item, i }) => {
              const track = toSlimTrack(
                TrackCodec.parse(item.track as unknown)
              );
          ...
          return ok(
            args.action,
            {
              ...
              limit:
                Number(json.limit ?? parsed.limit ?? 0) ||
        ...
        case "create": {
          ...
          const name = String(
            (jrec2?.name as string | undefined) ?? "playlist"
          );
        ...
        case "update_details": {
          ...
          return ok(
            args.action,
            { updated: true },
            `Updated playlist details for ${parsed.playlist_id}${detailsMsg}.`
          );
        }
        case "add_items": {
          if (!parsed.playlist_id)
            ...
            if (ids.length > 0) {
              ...
              trackNames = items
                .map((x) => {
                  ...
                  return parsedT.success
                    ? toSlimTrack(parsedT.data).name
                    : undefined;
                })
                .filter(Boolean) as string[];
            }
